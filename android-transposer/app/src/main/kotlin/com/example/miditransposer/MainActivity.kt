...
class MainActivity : Activity() {
    ...
    private fun refreshDevices() {
        ...
        // 某些手机驱动的 ports 列表信息不完整，因此同时检查输入端口数量。
        availableDevices = midiManager.devices.filter { info ->
            info.inputPortCount > 0 || info.ports.any {
    ...
    private fun openSelectedDevice() {
        ...
        midiManager.openDevice(info, { device ->
            ...
            if (midiInputPort == null) {
                val detail = openError?.takeIf { it.isNotBlank() } ?: "端口被占用或驱动不兼容"
                midiStatus.text = "MIDI：打开失败（输入端口 ${info.inputPortCount} 个）：$detail"
                midiStatus.setTextColor(Color.RED)
            } else {
